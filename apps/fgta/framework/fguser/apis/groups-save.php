...
if (is_file(__DIR__ .'/data-groups-handler.php')) {
	require_once __DIR__ .'/data-groups-handler.php';
}
...
/**
 * fgta/framework/fguser/apis/groups-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel groups fguser (fgt_usergroups)
 *
...
$API = new class extends fguserBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fgt_usergroups';
		$primarykey = 'usergroups_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\fguser_groupsHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fguser_groupsHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$header_table = 'fgt_user';
				$header_primarykey = 'user_id';
				...
				// result
				$options->criteria = [
					"usergroups_id" => $obj->usergroups_id
				];

				$criteriaValues = [
					"usergroups_id" => " usergroups_id = :usergroups_id "
				];
				...
				$sqlFieldList = [
					'usergroups_id' => 'A.`usergroups_id`', 'usergroups_isdisabled' => 'A.`usergroups_isdisabled`', 'group_id' => 'A.`group_id`', 'user_id' => 'A.`user_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fgt_usergroups A";
				$sqlWhere = $where->sql;
